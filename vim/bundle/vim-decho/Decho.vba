...
" Maintainer:  Charles E. Campbell, Jr. PhD <NdrOchip@ScampbellPfamily.AbizM>
" Date:        Aug 07, 2012
" Version:     21n	ASTRO-ONLY
"
" Usage: {{{1
"   Decho "a string"
...
fun! DechoOn(line1,line2)
  ...
  set noic
  let swp    = SaveWinPosn(0)
  let dbgpat = '\<D\%(echo\|echoWF\|func\|redir\|ret\|echo\%(Msg\|Rem\|Tab\|Var\)O\%(n\|ff\)\|echoToggle\)\>'
...
fun! DechoOff(line1,line2)
  ...
  let swp=SaveWinPosn(0)
  let swp= SaveWinPosn(0)
  exe "sil! keepj ".a:line1.",".a:line2.'g/\<D\%(echo\|echoWF\|func\|redir\|ret\|echo\%(Msg\|Rem\|Tab\|Var\)O\%(n\|ff\)\|echoToggle\)\>/s/^[^"]/"&/'
  call RestoreWinPosn(swp)
  let &ic= ickeep
endfun
...
5. Decho History					*decho-history*
  ...
  v21 Oct 31, 2008 : * improved DechoTab -- found a case where a new tab
      ...
      Dec 24, 2008   * using lz -- restore_lz around Decho when using
		       DechoTabOn
      Jan 19, 2009   * If a debugging tab is found, it still might not be
		       open to a "Decho Tab" window.  Append to the
		       "Decho Tab" window if its still present; otherwise,
		       regenerate the debugging tab and window.
      ...
      Jun 23, 2011   * uses the string() function when input argument is not a
		       string
